...
RUN if [[ "${TARGETARCH}" -eq "amd64" ]]; then CGO_ENABLED=1 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -ldflags '-s -w' -o /build/message-sender; else CGO_ENABLED=1 GOOS=${TARGETOS} GOARCH=${TARGETARCH} CC=/usr/bin/aarch64-alpine-linux-musl-cc go build -ldflags '-s -w' -o /build/message-sender; fi
